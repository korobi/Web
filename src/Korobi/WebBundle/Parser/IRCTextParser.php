...
class IRCTextParser {
    ...
    public static function parseLine($line, $pretty_only) {
        ...
        while (preg_match(self::INCREMENTAL_REGEX, $next, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            $result .= self::makeSafe(substr($next, 0, $index), false);
        ...
        $result .= self::makeSafe($next, false) . self::closeTags($styles);

        // Apply other transformations if needed
        if ($pretty_only) {
            return $result;
        } else {
            return self::transformUnsafe($result);
        }
    }
    ...
    public static function closeTag() {
        return str_replace(array_keys(self::$REPLACEMENTS), array_values(self::$REPLACEMENTS), '</span>');
    }
    ...
    private static function createFormatTag($style) {
        return str_replace(array_keys(self::$REPLACEMENTS), array_values(self::$REPLACEMENTS), '<span class="' . $style . '">');
    }
    ...
    public static function createColorTag($fg, $bg, $reverse = false) {
        return str_replace(array_keys(self::$REPLACEMENTS), array_values(self::$REPLACEMENTS), '<span class="'. ($reverse
    ...
    private static function transformUnsafe($text) {
        ...
        return str_replace(array_values(self::$REPLACEMENTS), array_keys(self::$REPLACEMENTS), $result);
    }
    ...
    private static function createLinkTag($url, $content) {
        return str_replace(array_keys(self::$REPLACEMENTS), array_values(self::$REPLACEMENTS), sprintf('<a href="%s" target="_blank">%s</a>', self::makeSafe($url, false), self::makeSafe($content)));
    }
}
